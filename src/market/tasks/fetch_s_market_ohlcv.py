...
from src.market.tasks._ohlcv_aggregation import (
    _row_to_entity,
    _select_aggregated_bars_for_schedule_slots,
)

_TASK = "fetch_s_market_ohlcv"
...
def run(**context: object) -> int:
    ...
    with log_task_run(_TASK) as logger:
        ...
        logger.info("target assets=%d filter=is_tradable=1 AND is_target=1", len(target_assets))
